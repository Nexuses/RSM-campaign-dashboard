...
export async function getGoogleSheetsClient() {
  // Option 1: Service Account Key as JSON (Recommended - supports full JSON key)
  // Requires GOOGLE_SERVICE_ACCOUNT_KEY as a JSON string
  if (process.env.GOOGLE_SERVICE_ACCOUNT_KEY) {
    try {
      ...
      throw new Error('Invalid GOOGLE_SERVICE_ACCOUNT_KEY format. It must be a valid JSON string.');
    }
  }

  // Option 2: Service Account (Legacy - separate email and key)
  ...
  // Option 3: API Key (Simpler, but less secure - only works with public sheets)
  if (process.env.GOOGLE_API_KEY) {
    return google.sheets({ version: 'v4', auth: process.env.GOOGLE_API_KEY });
  }

  throw new Error('Google Sheets credentials not configured. Please set GOOGLE_SERVICE_ACCOUNT_KEY (as JSON), or GOOGLE_SERVICE_ACCOUNT_EMAIL and GOOGLE_PRIVATE_KEY, or GOOGLE_API_KEY in your environment variables.');
}
...
export async function getSheetData(spreadsheetId: string, range: string) {
  try {
    ...
    if (error.code === 400) {
      // 400 Bad Request from Google Sheets API
      const errorDetails = error.message || 'Invalid request to Google Sheets API';
      throw new Error(`Bad Request: ${errorDetails}. Please check your GOOGLE_SHEET_ID and sheet name.`);
...
export function convertToCampaignData(sheetData: any[], headers?: string[]) {
  ...
  if (headers && headers.length > 0) {
    ...
    // Check if data is already parsed (objects with header keys) or raw (arrays)
    // If first row is an object (not an array), it's already parsed and headers were removed
    const isAlreadyParsed = sheetData.length > 0 && !Array.isArray(sheetData[0]);
    const dataRows = isAlreadyParsed ? sheetData : sheetData.slice(1); // Only slice if it's raw array data
    
    return dataRows.map((row) => {
      // Ensure row is an array for mapRowToCampaignData
